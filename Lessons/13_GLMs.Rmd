...
The coefficients of a hierarchical model are divided into two categories: **fixed effects** and **random effects.** A **fixed effect** is a factor whose levels are experimentally determined or whose interest lies in the effects of each level (e.g., covariates, treatments, interactions). A **random effect** is (must be) a factor whose levels are sampled from a larger population, or whose interest lies in the variation among them rather than the specific effect of each level. In choosing whether you are dealing with a fixed or a random effect, consider the following questions: 
...
TempTest.mixed <- lme(data = NTL.summertemp,
                     temperature_C ~ year4, 
                     random = ~1|lakename) # Sets random effect to lakename
summary(TempTest.mixed)
rsquared(TempTest.mixed) # Marginal is just fixed effects, Conditional is R2 including RE

# Compare the random effects model with the fixed effects model
TempTest.fixed <- gls(data = NTL.summertemp, # gls includes add'l info found in lme output
                      temperature_C ~ year4)
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  ...
  theme(legend.spacing.x = unit(0, "cm")) # Make legend labels appear closer on graph (removes   spaces)
print(NTL.tempmodel)
```

Question: How would you interpret the collective results of your mixed effects model in the context of the study question?

> ANSWER: There is a statistically significant increase in surface temperatures over time of 0.06 degrees Celsius per year (random effects model, random effect = lakename, p < 0.0001, R2 = 0.11)
