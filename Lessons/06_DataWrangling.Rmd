...
> Add notes here: These factors can affect local food chains and ecosystems, so these can indicate ecosystem health. They can also impact algal blooms. There can be long-term monitoring of climate change impacts.
...
NTL.phys.data.1990thru1999 <- filter(NTL.phys.data, year4 >= 1990 & year4 <= 1999 &
                                     lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> Answer: Filtering by row numbers isn't as reproducible and requires a lot more thinking to figure out what rows to use. Also, there might not be an even number of samples per year.
...
Selecting allows us to choose certain columns (variables) in our dataset. Can exclude columns using a negative sign (but can't mix inclusion and exclusion)
...
> Answer: For reproducibility and so you can recreate the processed code if it gets messed up. This is also useful if you want to send the processed data to someone else to run their own analysis.
