...
EnoDischarge.complete <- EnoDischarge %>%
  drop_na(discharge.mean) # Drops only rows with NA in discharge.mean column
...
```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], frequency = 365) #[[]] lets you choose column - ts doesn't like $name format
```
...
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic") # Needs ts, not dataframe
...
ggplot(EnoDischarge.late_Components) +
  ...
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff", size = 1.5) +
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Discharge (ft"^3*"/s)"))

# Visualize how the seasonal cycle maps onto the data - Note: seasonal flow goes below 0!
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
...
* **Kruskal-Wallis test**: non-parametric version of t-test (Wilcox test)
...
```{r}
EnoDischarge.early.subsample <- sample_n(EnoDischarge.early, 5000) # Necessary for shapiro
EnoDischarge.late.subsample <- sample_n(EnoDischarge.late, 5000)

shapiro.test(EnoDischarge.early.subsample$discharge.mean) # Can only take up to 5000 points
...
> The means of the early and late subgroups are statistically significantly different from each other, with the early period having a higher discharge than the later period (Kruskal-Wallis, W = 1.17*10^8, p < 0.0001). A boxplot of discharge for the two periods might work for visualization.

### Monotonic trend analysis (Start of Lesson on 2/27/2020)
...
> aggregation: Combining data to create data for a coarser timeframe (e.g., combine monthly data to make yearly data).

> subsampling: Selecting a smaller set of a larger data set (e.g., taking a random sample from daily data for each month, or taking the first day of each month to get monthly data).

> interpolation: Filling in data gaps based on known data
...
EnoDischarge.late.monthly$Date <- as.Date(paste(EnoDischarge.late.monthly$Year, 
                                                ...
                                                1, sep="-"), # Placeholder date on the first of the month (to make plotting easier/look better)
                                          format = "%Y-%m-%d") 
...
EnoDischarge.late.monthly.ts <- ts(EnoDischarge.late.monthly$Discharge, frequency = 12, 
                        start = c(1985, 10, 1), end = c(2019, 12, 1)) # Specify start and end dates since we aren't using actual dates (timeseries is just a single column/list of data)

# Run SMK test (requires timeseries object, not dataframe)
...
> Based on these results, there is no significant trend over time in the Eno River discharge (Seasonal Mann Kendall Test, z = -0.1397, p = 0.8889).
