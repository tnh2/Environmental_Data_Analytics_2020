...
mytheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), 
        legend.position = "top")
theme_set(mytheme) # Can you "save" a theme?
```

### One-way ANOVA
A one-way ANOVA is the same test in practice as a two-sample t-test but for three or more groups. In R, we can  run the model with the function `lm` or `aov`, the latter of which which will allow us to run post-hoc tests to determine pairwise differences. ANOVA lets you know if there is significant difference between the group means (i.e., is that grouping a significant predictor of output change).

Does total litter biomass differ among sites at the Niwot Ridge LTER?
```{r}
# Wrangle the data
Litter.Totals <- Litter %>%
  group_by(plotID, collectDate, nlcdClass) %>% # What does group_by do, exactly?
...
summary(Litter.Totals.anova2) # In this case, intercept is first grouping (alphabetically)

# Checking model fit and assumptions
# ANOVA is robust against departures from normality.
plot(Litter.Totals.anova2) # Can you get it to plot just one of the plots?

# Post-hoc test
TukeyHSD(Litter.Totals.anova) # Only works on "aov" object

# Extract groupings for pairwise relationships - shows which statistical group a level is in (levels in the same group are not statistically different from each other)
...
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  ...
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 3.5,
               label = c("abc", "ab", "abcd", "bcd", "bcd", "a", 
                         "d", "abcd", "cd", "d", "abcd", "abcd")) + # Assign labels in the order it will be on the plot
...
Litter.interaction <- with(Litter, interaction(functionalGroup, nlcdClass)) # Finds interaction between two groupings
...
stat.letters <- data.frame(match(rownames(Litter.groups$groups),levels(Litter.interaction)),
                  as.character(Litter.groups$groups[[2]]))
colnames(stat.letters) <- c("Order", "Letter")
rownames(stat.letters) <- rownames(Litter.groups$groups)
stat.letters <- stat.letters[order(stat.letters$Order),]

Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  geom_boxplot() +
  labs(x = "nlcd Class", y = "Mass (g)", color = "") +
  scale_colour_ordinal() +
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 3.5,
               position = position_dodge(0.7),
               label = as.character(stat.letters$Letter))
