...
# Fix this code (take color out of aes() statement)
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) # Breakout facets by Site.Name
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
                  Site.Name == "Bryson City"), # Choose only entries for three sites
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) # Create a facet grid based on Site.Name (row) and Year (col)
...
TempPlot1 <- ggplot(subset(PeterPaul.chem.nutrients, depth == 0),
...
TempPlot2 <- ggplot(PeterPaul.chem.nutrients) +
...
# Adjust axes keeps outliers from messing stuff up
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) +
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # Reverses scale on y-axis
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE) + # Draws a line of best fit (without std. error)
  geom_abline(aes(slope = 16, intercept = 0)) # Draws a straight line
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? - Color also colors the dots
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) # Dodge offsets points with the same values (so identical values are wide)
print(Nutrientplot4)
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) # Alpha is transparency
print(Nutrientplot6)
