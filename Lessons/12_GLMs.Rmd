...
getwd()
library(tidyverse)
options(scipen = 4) # Displays decimals more often than scientific notation
...
>  Irradiance decreases by 95.89 units for each 1 m increase in depth (linear regression, R2 = 0.31, df = 15,449, p < 0.0001,)
...
par(mfrow = c(2,2), mar=c(1,1,1,1)) # mar helps knit correctly
...
```{r, fig.height = 3, fig.width = 4}
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, # Remove 0 values and oultiers
                                   irradianceWater != 0 & irradianceWater < 5000)
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth)
summary(irradiance.regression2) # Coefficients have to be interpreted differently
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + # Similar to using log(y-variable) but makes y-axis more intuitive
  geom_point() 
...
corrplot.mixed(PeterPaulCorr, upper = "ellipse") # Plots values on mirror half of matrix
```
...
step(TPAIC) # Computes AIC for model with each independent variable removed, iteratively
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)

```

> Dissolved oxygen, temperature, and total nitrogen all have a significant effect on total nitrogen (multiple linear regression, R2 = 0.26, DF = 109, p < 0.0001). The model containing these three explanatory variables was the most parsimonious of the models explored.