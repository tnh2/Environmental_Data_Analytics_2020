...
```{r, message = FALSE}
getwd()
library(plyr) # Important to load this first since we want dplyr to mask some of these functions
library(tidyverse)
library(lubridate)
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% # Use this dataset for pipe
  mutate(month = month(sampledate)) %>% # Creates a column with the month number
  select(lakeid:daynum, month, sampledate:comments) %>% # Rearrange order of columns/fields
  drop_na(depth) # Removes entries (rows) with 'NA' listed for depth
...
# Alternatively, use select(NTL.nutrient.data.PeterPaul, -lakeid, -depth_id, -comments)
```
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.subset, 
                                               "nutrient", "concentration", tn_ug:po4)
...
# Split components of cells into multiple columns (doesn't preserve initial column)
...
LitterFiles # Creates a list of all the file names ending in ".csv" in the NIWO_Litter folder

Litter <- LitterFiles %>%
  ldply(read.csv) # Creates dataframe by reading in all the files listed in LitterFiles
```
...
NTL.PeterPaul.summaries <- 
  NTL.phys.nutrient.data.PeterPaul %>% # Take this dataframe
  filter(depth == 0) %>% # Look only at rows with depth of 0
  group_by(lakename, month) %>% # Group by lakename and month (row for each lake per month)
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% # Remove rows with NAs in all three of these fields
  summarise(meantemp = mean(temperature_C), # Create new columns for each of these fields
