...
theme_set(mytheme) # Sets theme for all subsequent plots
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + # Creates a horizontal line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) +
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +
  geom_text(x = as.Date("2020-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") + # Adds text at a certain height and justified right
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1, fontface = "bold") +
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), # Scales the dates axis
    date_breaks = "2 months", date_labels = "%b %y") + # Sets the intervals of the axis labels and formats dates
  ylab(expression("O"[3]* " AQI Value")) + # Formats axis labels (with subscript on O3)
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) # theme adjustment
print(O3plot7)  
...
scales::show_col(colormap(colormap = colormaps$viridis, nshades = 16)) # Displays hex-codes
...
NvsP2 <-
  ...
  labs(x = expression(paste("Total Phosphorous, ", mu, "g/L")),
       y = expression(paste("Total Nitrogen, ", mu, "g/L")),
       color = "Depth, m", shape = "Lake") + # change your legend labels here
