...
```{r check.pdf}
pdf(here("outputs", "pdf_test.pdf"), width = 11, height = 8.5) # Saves as a pdf
ggplot(data = cars) +
...
```{r activity.here}
# test out here
here() # Similar to working directory
...
st_crs(basins_nf_seplains) <- my_epsg
...
# add your code here
NC_bounds_geometry <- southeast_state_bounds %>%
  filter(NAME == "North Carolina") %>%
  st_geometry() # Only takes geometry data (not tabular data)

# check
# add your code here
class(NC_bounds_geometry)
NC_bounds_geometry

# select watersheds that intersect with NC bounds
# add your code here
NC_watersheds <- st_intersection(basins_nf_seplains, NC_bounds_geometry)

# check
# add your code here
head(NC_watersheds)
summary(NC_watersheds)

# select gages that fall within NC bounds
# add your code here
NC_gages <- st_intersection(gages, NC_bounds_geometry)
head(NC_gages)
summary(NC_gages)

# bonus
# add your code here
NC_area <- st_area(NC_bounds_geometry) # find the area of NC
NC_centroid <- st_centroid(NC_bounds_geometry) # find the centroid of the NC
NC_gages_buffer_100m <- st_buffer(NC_gages, dist = 100) # make a buffer around the stream gages in NC
not_NC_gages <- st_difference(gages, NC_bounds_geometry) # find stream gages not in NC
...
An important part of spatial data analysis is to *check your work* and also *share your results*. The sf package makes this very easy by adding a geom_sf() to add a vector data layer and geom_sf_text() to add a label layer to any ggplot2 object.
...
# basic plot
pdf(here("outputs", "NC_bounds.pdf"), width = 11, height = 8.5)
ggplot() +
  geom_sf(data = NC_bounds) + # you can add color = "blue" to change outline color to blue or lwd = 2 to change the width of the lines
  theme_bw() # add this to make the background white and not grey
dev.off()
...
ggplot() +
  ...
  geom_sf(data = NC_watersheds, fill = "salmon", alpha = 0.25) +
...
ggmap(nc_basemap) + # Using ggmap - sets CRS to input data (nc_basemap)
